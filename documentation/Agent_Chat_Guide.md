...
## SageLLM Chatbot v2.0.0 (`models/sagellm_chatbot.sage`)
...
```bash
sage models/build_sagellm.sage
```

This runs data collection, model init, pre-training, LoRA fine-tuning, DPO, RAG, Engram memory, quantization, chatbot generation, GGUF export, visualization, and summary — producing `models/sagellm_chatbot.sage` as its output.
