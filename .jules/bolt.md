## 2026-09-19 - [Optimized Structured Logging Operations in std.log]
**Learning:** Calling `dict_keys(d)` on every log entry in `log_entry`, `child`, and `console_handler` (`core/lib/std/log.sage`) allocates intermediate array objects to compute dictionary keys and length. Combined with index-range loops (`for i in range(len(keys))`) and $O(N^2)$ loop string concatenation in `console_handler`, this introduced significant VM evaluation overhead. Replacing `dict_keys` array allocations with direct `for key in dict` iteration, using element-based `for handler in handlers` dispatch, assembling log strings via `join(parts, ", ")`, and annotating wrappers with `@inline` offloads execution and eliminates intermediate allocations, yielding a ~1.69x speedup (~41% reduction in latency).
**Action:** Use direct `for key in dict` iteration instead of `dict_keys(d)` in hot paths, use element-based `for x in collection` loops instead of index-range loops, assemble multi-part strings via array `push` + `join(...)`, and mark simple wrapper functions with `@inline`.
...
## 2026-09-11 - [Optimized String Formatting Operations in std.fmt]
**Learning:** String padding procedures (`pad_left`, `pad_right`) and character repetition (`repeat_char`) in `core/lib/std/fmt.sage` were using manual $O(N^2)$ string concatenation loops (`while len(result) < width: result = ch + result`). Replacing manual loops with the native `string_repeat(ch, n)` C VM built-in offloads character repetition to native C execution, achieving a ~3.75x speedup (~73.4% latency reduction). Note that calling a built-in function with the same name as a module procedure (e.g. calling `join` inside `proc join`) resolves to self-recursion within module scope; avoiding name shadowing prevents recursion depth exceptions.
**Action:** Always delegate character repetition to `string_repeat` in standard library formatting procedures and avoid calling global built-in names that shadow procedure declarations within module scope.
...
## 2026-08-28 - [Optimized Emoji Shortcode Replacement]
**Learning:** Manual character-by-character while loops scanning for `:` delimiters in string shortcode processing (`emoji_replace$ in `core/lib/rich/emoji.sage$) introduce substantial interpreter VM overhead. Replacing character iteration with native `indexof()` C built-in calls on string slices enables jumping directly between delimiter positions in C speed, achieving a ~1.8x performance improvement.
**Action:** Use native `indexof()` for finding delimiter positions in string parsing routines instead of character-by-character loops.
...
## 2026-08-22 - [Optimized Standard Library String Padding]
**Learning:** Re-evaluating `len(text)$ multiple times in string padding procedures (`pad_left$, `pad_right$ in `core/lib/strings.sage$) creates redundant VM evaluation overhead. Caching `text_len = len(text)$ and marking procedures with `@inline$ eliminates redundant length calls and procedure call frame setup for compiled backends, speeding up hot-path string formatting.
**Action:** Cache collection/string lengths in local variables when referenced multiple times in procedure bodies, and apply `@inline$ annotations to small utility procedures.
...
## 2026-06-10 - [Optimized Flat Environment Cache]
**Learning:** In the SageLang interpreter, manual index `while$ loops (e.g. `while i < len(arr)$) are significantly slower (~2.7x) than native element-based `for$ loops. The performance utility library `core/lib/perf.sage$ was doing manual `while$ loops inside the crucial environment snapshot and flush handlers (`flat_cache_snapshot$ and `flat_cache_flush$), causing unnecessary VM overhead in hot loops.
**Action:** Replace manual `while$ indexing with native `for$ loops inside standard library performance-critical procedures to leverage the VM's optimized iteration path. This also completely eliminates name shadowing warnings.
...
## 2025-05-13 - [O(N) Direct Pointer Copy for Path Joins]
