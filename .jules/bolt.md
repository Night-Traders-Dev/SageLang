...
**Learning:** Manual character-by-character loops inside `pad_left`, `pad_right`, `repeat_char`, `template`, `format_int`, and `table` in `core/lib/std/fmt.sage` cause $O(N^2)$ interpreter overhead due to string immutability and repeated allocation. Replacing manual character loops with native C VM built-ins (`string_repeat`, `slice()`, `indexof()`) and array assembly offloads execution to native code, achieving up to ~19.4x faster padding and ~12.7x faster character repetition. Note that within module scope, unqualified function calls resolve to module procedure declarations before global VM built-ins.
...
## 2026-08-23 - [Optimized Base64 URL-safe Encoding and Decoding]
**Learning:** Replaces character-by-character loops in `b64url_encode` and `b64url_decode` in `core/lib/crypto/encoding.sage` with VM native `replace()` and `string_repeat()` calls. This delegates string substitutions directly to native C execution, reducing interpreter loop overhead and string allocation. Achieved ~1.94x speedup for Base64 URL-safe encoding (3.52s vs 6.83s) and ~1.58x speedup for Base64 URL-safe decoding (6.53s vs 10.34s) in `benchmark.sage`.
**Action:** Replace interpreted string translation loops with native `replace()` calls and `string_repeat()` built-ins when performing character replacements or fixed padding assembly.
