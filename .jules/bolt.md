...
## 2026-09-04 - [Optimized Standard Library Unicode String Operations]
**Learning:** Manual character-by-character loops inside `to_upper`, `to_lower`, `trim`, `trim_left`, `trim_right`, `repeat_str`, `reverse`, `starts_with`, `ends_with`, `to_title`, `swap_case`, and `center` in `core/lib/std/unicode.sage` cause high $O(N^2)$ interpreter overhead due to repeated string allocations and invalid string index calls (`s[i]`). Delegating manual loops to native C VM built-ins (`upper`, `lower`, `strip`, `slice`, `string_repeat`, `startswith`, `endswith`, `string_count`) offloads string operations to native code, achieving >1000x speedup (e.g. `trim` went from ~0.841s to ~0.00077s and `to_upper` went from ~1.176s to ~0.00089s).
**Action:** Always delegate standard string operations to native C VM built-ins (`upper`, `lower`, `strip`, `string_repeat`, `slice`, `startswith`, `endswith`) in standard library modules.
