## 2026-09-21 - [Optimized URL-Safe Base64 Encoding and Decoding]
**Learning:** In `core/lib/crypto/encoding.sage`, `b64url_encode` and `b64url_decode` used character-by-character `for` loops and intermediate array construction (`parts = []`, `push()`, `join()`) to replace `+` with `-` and `/` with `_`. Replacing manual character array assembly with native `replace()` VM built-in calls in C space and replacing iterative `=` string append loops with `string_repeat()` offloads string transformations to native C execution, achieving ~1.71x faster URL-safe Base64 encoding (~3.93s vs 6.72s) and ~1.59x faster decoding (~7.17s vs 11.38s).
**Action:** Use native `replace()` VM built-ins for character translation and `string_repeat()` for string padding instead of character-by-character array building loops in encoding and string manipulation routines.
