...
static void shell_exec(const char *cmd) {
    ...
    if (streq(cmd, "mem")) {
        term_write("\nKernel loaded at physical 0x00100000");
        term_write("\nStack size: 64 KiB");
        term_write("\nMemory map handoff: pending");
        return;
    }
