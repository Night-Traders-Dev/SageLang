...
proc nlmsg_serialize(msg_in):
    ...
    let total = 16
    # len as LE u32
    push(bytes, total % 256)
    push(bytes, (total / 256) % 256)
    push(bytes, 0)
    push(bytes, 0)
    # type as LE u16
    push(bytes, msg_in["type"] % 256)
    push(bytes, (msg_in["type"] / 256) % 256)
    # flags as LE u16
    push(bytes, msg_in["flags"] % 256)
    push(bytes, (msg_in["flags"] / 256) % 256)
    # seq as LE u32
    push(bytes, msg_in["seq"] % 256)
    push(bytes, 0)
    push(bytes, 0)
    push(bytes, 0)
    # pid as LE u32
