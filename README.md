...
## Recent Updates
- **v4.1.14 (Self-Hosted CLI Parity, Proc Type Annotations & Anonymous Procs)**: Achieved full CLI flag surface parity in the self-hosted interpreter (`sage.sage` flags `-c`, `--jit`, `--aot`, `--sgvm`, `--lsp`, `--emit-kotlin`), added procedure parameter/return type annotation support in self-hosted compiler (`proc f(x: Int, y: Int): Int`), bare `end` nil-expression statement / optional block terminator support, anonymous procedure expressions (`proc(params...): body [end]`), and LSP formatting capabilities (`documentFormattingProvider`).
