...
## Recent Updates
- **v4.1.14 (Veritas Quality Audit & Standard Library Fixes)**: Audited test suites, resolved duplicate procedure definition in `crypto/hash.sage`, fixed invalid trailing `parse_rrs` calls in `net/dns.sage`, fixed string iteration in `metal/vga.sage`, verified 385 unit tests + 88 JSON tests pass, clean git status without runtime artifacts, and bumped patch version on success.
