...
## Recent Updates
- **v4.2.6 (Metal IRQ Safety, Regular Expression Optimization & Runtime Hardening)**: Added panic-free IRQ vector registration (`register_handler_safe`) and safe unregistration (`unregister_handler`) in `metal.irq`. Optimized `std.regex` character class compilation, search, and replacement routines via native C VM built-ins (`slice()`, `join()`, `indexof()`) for ~1.8x speedup, and accelerated Rich TUI style/color/markdown processing (`rich.color`, `rich.style`, `rich.markdown`). Hardened `sage_string_repeat` runtime helper against integer overflow (CWE-190) and resource exhaustion (CWE-400/789), and added size/NULL checks in `sgvm_compiler` and `sgvm_main`.
