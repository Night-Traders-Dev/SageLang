...
**Current version:** v4.1.14 · **Spec version:** 2.0 · **License:** MIT

## Recent Updates
- **v4.1.14 (CLI Parity & Self-Hosted Compiler Parity)**: Complete CLI flag surface parity with C `main.c` in self-hosted driver (`--lsp` with document formatting and version info, `-c 'code'`, `--jit <file>`, `--aot <file>` C parity printing/writing, `--sgvm` alias). Self-hosted parser syntax additions: bare `end` parsed as nil-expression statement (optional block terminator), anonymous `proc` expression parsing (`EXPR_PROC`) with optional trailing `end`, procedure parameter/return type annotations (`proc f(x: Int, y: Int): Int`).
- **v4.1.13 (Self-Hosted Compiler Parity: Type Annotations)**: Completed type annotation parsing support in the self-hosted compiler port (`core/src/sage/parser.sage`, `core/src/sage/ast.sage`, `core/src/sage/typecheck.sage`). Full support for `let x: Int = ...`, `proc f() : Int`, and `proc f(x: Int)` with declared/inferred type tracking via `TypeMap.declared` dict and `annotation_to_kind` mapping. Safety checks enhanced with `check_sync`, `@safe` doc tracking, and `EXPR_PROC` body analysis. constfold improved with inf/NaN guard and 64KB string concat cap. All self-hosted tests pass (430+ tests). RISC-V suite: 56 pass/50 fail (pre-existing). SVM: 106/106.

- **v4.1.11 (String Interner & Memory Leak Fix)**: Fixed unbounded memory growth (~140MB/s) in long-running RISC-V programs by adding a content-keyed string interner (`sage_string_const`) to the AOT runtime in `core/src/c/compiler.c`. Compile-time constant strings (literals and property-access keys like "pc"/"bytecode"/"trace") are now interned and rooted from the GC, eliminating per-instruction heap allocations. Self-hosted compiler type annotation infrastructure added (see Self-Hosting Guide). RISC-V suite: 56 pass/50 fail (all 50 verified pre-existing on Aug-11 binary); SVM: 106/106.
- **v4.1.12 (Self-Hosted Compiler Parity: Type Annotations)**: Added type annotation infrastructure to the self-hosted Sage compiler port (`core/src/sage/typecheck.sage`). Supports `let x: Int = ...`, `proc f() : Int`, and `proc f(x: Int)` with declared/inferred type tracking via `TypeMap.declared` dict and `annotation_to_kind` mapping (mirrors C compiler's annotation system). Bootstrap test: 18/18 pass. Further parity work in progress: safety checks, constfold improvements, linter rules, env enhancements.
...
- **v4.1.9 (Veritas Quality Audit & Submodule Stabilization)**: Previous release with Veritas quality audit.
