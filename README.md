...
## Recent Updates
- **v4.2.6 (CommonJS → ESM Transpiler Submodule Addition)**: Integrated the `cjs2esm` CommonJS-to-ESM transpiler submodule (`core/lib/transpiler/cjs2esm/`), providing automated JavaScript AST analysis, CommonJS `require`/`module.exports` transformation to ES module syntax (`import`/`export`), project file conversion, and migration report generation via `cjs2esm` CLI tool.
