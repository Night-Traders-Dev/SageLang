...
| **GGUF** | `import llm.gguf` | GGUF v3.8.3 export for Ollama and llama.cpp compatibility |
...
- **Module imports**: `import X`, `import X as Y`, `from X import a, b` with module caching and multi-path search (`./`, `lib/`). Supports `__init__.sage` for directory-based packages (v3.8.3+).
- **Soft Keywords**: `match`, `init`, `enum`, `struct`, and `trait` are now "soft keywords" and can be used as variable names in expressions and assignments (v3.8.3+)
...
- **`gguf`**: GGUF v3.8.3 export for Ollama and llama.cpp (metadata, Modelfile, quantization, conversion scripts)
