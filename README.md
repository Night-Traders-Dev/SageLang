...
## Recent Updates
- **v4.2.6 (Documentation Audit & System Parity Sync)**: Synchronized standard library and bare-metal documentation, soft keyword parity across specification guides (`print`, `end`, `match`, `init`, `enum`, `struct`, `trait`), updated ESP32 microcontroller board listings, and verified CLI option references (`sage safety`, `--strict-safety`, `--sandbox`, `--run-vm`, `-I <dir>`, `--ois`).
