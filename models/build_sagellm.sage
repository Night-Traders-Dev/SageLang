...
emit("    let chain = {"steps": [], "conclusion": ""}")
...
emit("        push(chain["steps"], {"type": "thought", "content": " + DQ + "I recall: " + DQ + " + mem_text)")
emit("    else:")
emit("        push(chain["steps"], {"type": "thought", "content": " + DQ + "No specific memory found, using general knowledge" + DQ + ")")
emit("    # Step 2: Classify the question (specific topics first, then general)")
emit("    let topic = " + DQ + "general" + DQ)
emit("    # --- Specific domain topics (check FIRST) ---")
emit("    if contains(lp, " + DQ + "llm" + DQ + ") or contains(lp, " + DQ + "language model" + DQ + ") or contains(lp, " + DQ + "transformer" + DQ + ") or contains(lp, " + DQ + "tokeniz" + DQ + ") or contains(lp, " + DQ + "attention" + DQ + ") or contains(lp, " + DQ + "lora" + DQ + ") or contains(lp, " + DQ + "engram" + DQ + ") or contains(lp, " + DQ + "neural" + DQ + "):")
emit("        topic = " + DQ + "llm" + DQ)
emit("    if contains(lp, " + DQ + "agent" + DQ + ") or contains(lp, " + DQ + "react" + DQ + ") or contains(lp, " + DQ + "tool use" + DQ + ") or contains(lp, " + DQ + "autonomous" + DQ + ") or contains(lp, " + DQ + "scratchpad" + DQ + "):")
...
emit("    if contains(lp, " + DQ + "crypto" + DQ + ") or contains(lp, " + DQ + "sha" + DQ + ") or contains(lp, " + DQ + "hash" + DQ + ") or contains(lp, " + DQ + "encrypt" + DQ + ") or contains(lp, " + DQ + "base64" + DQ + ") or contains(lp, " + DQ + "hmac" + DQ + "):")
...
emit("    if contains(lp, " + DQ + "baremetal" + DQ + ") or contains(lp, " + DQ + "uefi" + DQ + ") or contains(lp, " + DQ + "kernel" + DQ + ") or contains(lp, " + DQ + "elf" + DQ + ") or contains(lp, " + DQ + "pci" + DQ + ") or contains(lp, " + DQ + "acpi" + DQ + ") or contains(lp, " + DQ + "osdev" + DQ + "):")
emit("        topic = " + DQ + "osdev" + DQ)
emit("    if contains(lp, " + DQ + "tensor" + DQ + ") or contains(lp, " + DQ + "machine learn" + DQ + ") or contains(lp, " + DQ + "training" + DQ + ") or contains(lp, " + DQ + "neural net" + DQ + ") or contains(lp, " + DQ + "optimizer" + DQ + ") or contains(lp, " + DQ + "gradient" + DQ + "):")
emit("        topic = " + DQ + "ml" + DQ)
...
emit("    if contains(lp, " + DQ + "gpu" + DQ + ") or contains(lp, " + DQ + "vulkan" + DQ + ") or contains(lp, " + DQ + "opengl" + DQ + ") or contains(lp, " + DQ + "shader" + DQ + ") or contains(lp, " + DQ + "render" + DQ + "):")
emit("        topic = " + DQ + "graphics" + DQ)
emit("    # --- General language topics ---")
...
emit("    if topic == " + DQ + "general" + DQ + ":")
emit("        if contains(lp, " + DQ + "gc" + DQ + ") or contains(lp, " + DQ + "garbage" + DQ + "):")
emit("            topic = " + DQ + "gc" + DQ)
emit("    if topic == " + DQ + "general" + DQ + ":")
emit("        if contains(lp, " + DQ + "compile" + DQ + ") or contains(lp, " + DQ + "backend" + DQ + ") or contains(lp, " + DQ + "emit" + DQ + "):")
...
emit("    push(chain["steps"], {"type": "thought", "content": " + DQ + "Topic classified as: " + DQ + " + topic)")
emit("    # Step 3: Generate answer based on topic")
emit("    let answer = " + DQ + DQ)
emit("    # --- Domain-specific answers ---")
emit("    if topic == " + DQ + "llm" + DQ + ":")
emit("        answer = " + DQ + "The Sage LLM library (lib/llm/) has 12 modules for building language models:" + DQ + " + chr(10) + chr(10) + " + DQ + "  import llm.config      # Model configs: tiny (1M) to Llama-13B" + DQ + " + chr(10) + " + DQ + "  import llm.tokenizer    # BPE, character, and word tokenizers" + DQ + " + chr(10) + " + DQ + "  import llm.embedding    # Token + positional (sinusoidal, learned, RoPE)" + DQ + " + chr(10) + " + DQ + "  import llm.attention    # Multi-head self-attention, KV cache" + DQ + " + chr(10) + " + DQ + "  import llm.transformer  # Full transformer: LayerNorm, RMSNorm, FFN, SwiGLU" + DQ + " + chr(10) + " + DQ + "  import llm.generate     # Greedy, top-k, top-p, beam search, repetition penalty" + DQ + " + chr(10) + " + DQ + "  import llm.train        # Training loops, cosine LR, cross-entropy, perplexity" + DQ + " + chr(10) + " + DQ + "  import llm.agent        # Tool use, CoT reasoning, memory, multi-agent teams" + DQ + " + chr(10) + " + DQ + "  import llm.prompt       # ChatML, Llama, Alpaca formats, templates, few-shot" + DQ + " + chr(10) + " + DQ + "  import llm.lora         # Low-rank adapters for efficient fine-tuning" + DQ + " + chr(10) + " + DQ + "  import llm.quantize     # Int8/int4 compression with error analysis" + DQ + " + chr(10) + " + DQ + "  import llm.engram       # 4-tier persistent memory (working/episodic/semantic/procedural)" + DQ + " + chr(10) + chr(10) + " + DQ + "Plus ml_native (C backend) for fast matmul, softmax, cross_entropy, adam." + DQ)
emit("    if topic == " + DQ + "agent_framework" + DQ + ":")
emit("        answer = " + DQ + "The Sage agent framework (lib/agent/) provides autonomous AI agents:" + DQ + " + chr(10) + chr(10) + " + DQ + "  import agent.core      # ReAct loop: observe -> think -> act -> reflect" + DQ + " + chr(10) + " + DQ + "  import agent.tools     # File I/O, code analysis, search, system tools" + DQ + " + chr(10) + " + DQ + "  import agent.planner   # Task decomposition with dependency DAG" + DQ + " + chr(10) + " + DQ + "  import agent.router    # Multi-agent orchestrator, capability routing" + DQ + " + chr(10) + chr(10) + " + DQ + "Agents parse LLM responses for TOOL: and ANSWER: commands." + DQ + " + chr(10) + " + DQ + "Chain-of-thought reasoning via scratchpad (think/observe/act/reflect)." + DQ + " + chr(10) + " + DQ + "Planning breaks goals into steps with tool assignments and dependencies." + DQ)
emit("    if topic == " + DQ + "chatbot" + DQ + ":")
emit("        answer = " + DQ + "The Sage chatbot framework (lib/chat/) provides:" + DQ + " + chr(10) + chr(10) + " + DQ + "  import chat.bot        # Conversation, intents, middleware, LLM responses" + DQ + " + chr(10) + " + DQ + "  import chat.session    # Multi-session store, history, text/JSON export" + DQ + " + chr(10) + " + DQ + "  import chat.persona    # 6 built-in: SageDev, CodeReviewer, Teacher, Debugger, Architect, Assistant" + DQ + " + chr(10) + chr(10) + " + DQ + "Intents: keyword-based matching with scoring." + DQ + " + chr(10) + " + DQ + "Middleware: pre/post processing pipeline for messages." + DQ + " + chr(10) + " + DQ + "Personas: apply_persona(bot, persona.sage_developer())" + DQ)
emit("    if topic == " + DQ + "crypto" + DQ + ":")
emit("        answer = " + DQ + "The crypto library (lib/crypto/) provides:" + DQ + " + chr(10) + chr(10) + " + DQ + "  import crypto.hash     # SHA-256, SHA-1, CRC-32" + DQ + " + chr(10) + " + DQ + "  import crypto.hmac     # HMAC with pluggable hash, constant-time compare" + DQ + " + chr(10) + " + DQ + "  import crypto.encoding # Base64 (standard + URL-safe), hex" + DQ + " + chr(10) + " + DQ + "  import crypto.cipher   # XOR, RC4, PKCS7 padding, CBC/CTR modes" + DQ + " + chr(10) + " + DQ + "  import crypto.rand     # xoshiro256** PRNG, UUID v4, shuffle" + DQ + " + chr(10) + " + DQ + "  import crypto.password # PBKDF2-HMAC, password hash/verify" + DQ)
emit("    if topic == " + DQ + "networking" + DQ + ":")
emit("        answer = " + DQ + "Sage networking: native modules + lib/net/ high-level suite:" + DQ + " + chr(10) + chr(10) + " + DQ + "  Native: import socket, tcp, http, ssl" + DQ + " + chr(10) + chr(10) + " + DQ + "  import net.url         # URL parse/build, percent encoding, query strings" + DQ + " + chr(10) + " + DQ + "  import net.headers     # HTTP header parse/build, content-type" + DQ + " + chr(10) + " + DQ + "  import net.request     # HTTP client builder, auth, response helpers" + DQ + " + chr(10) + " + DQ + "  import net.server      # TCP/HTTP server, routing, response builders" + DQ + " + chr(10) + " + DQ + "  import net.websocket   # WebSocket frames (RFC 6455), upgrade" + DQ + " + chr(10) + " + DQ + "  import net.mime        # 80+ MIME types from file extensions" + DQ + " + chr(10) + " + DQ + "  import net.dns         # DNS wire-format parse/build" + DQ + " + chr(10) + " + DQ + "  import net.ip          # IPv4, CIDR, private/loopback/multicast" + DQ)
emit("    if topic == " + DQ + "osdev" + DQ + ":")
emit("        answer = " + DQ + "The OS dev library (lib/os/, 15 modules) for baremetal/kernel/UEFI:" + DQ + " + chr(10) + chr(10) + " + DQ + "  import os.fat / os.fat_dir   # FAT filesystem + directory traversal" + DQ + " + chr(10) + " + DQ + "  import os.elf / os.pe        # ELF + PE/COFF binary parsers" + DQ + " + chr(10) + " + DQ + "  import os.mbr / os.gpt       # Partition table parsers" + DQ + " + chr(10) + " + DQ + "  import os.pci / os.acpi      # Hardware enumeration" + DQ + " + chr(10) + " + DQ + "  import os.uefi               # EFI memory map, RSDP, config tables" + DQ + " + chr(10) + " + DQ + "  import os.paging / os.idt    # Page tables, interrupt descriptors" + DQ + " + chr(10) + " + DQ + "  import os.serial             # UART/COM port for debug output" + DQ + " + chr(10) + " + DQ + "  import os.dtb                # Device tree for ARM64/RISC-V" + DQ + " + chr(10) + " + DQ + "  import os.alloc / os.vfs     # Kernel allocators, virtual filesystem" + DQ)
emit("    if topic == " + DQ + "ml" + DQ + ":")
emit("        answer = " + DQ + "Sage ML libraries (lib/ml/ + lib/cuda/ + ml_native):" + DQ + " + chr(10) + chr(10) + " + DQ + "  import ml.tensor    # N-dim tensors, matmul, activations, softmax" + DQ + " + chr(10) + " + DQ + "  import ml.nn         # Linear, ReLU, Sigmoid, Dropout, Sequential" + DQ + " + chr(10) + " + DQ + "  import ml.optim      # SGD (momentum), Adam, LR schedulers" + DQ + " + chr(10) + " + DQ + "  import ml.loss       # MSE, cross-entropy, Huber, L1, KL divergence" + DQ + " + chr(10) + " + DQ + "  import ml.data       # Dataset, DataLoader, normalization, train/test split" + DQ + " + chr(10) + chr(10) + " + DQ + "Native backend (ml_native): C-optimized matmul, softmax, RMSNorm, Adam." + DQ + " + chr(10) + " + DQ + "12+ GFLOPS on 64x64 matmul without BLAS." + DQ)
emit("    if topic == " + DQ + "regex" + DQ + ":")
emit("        answer = " + DQ + "Sage regex engine (import std.regex):" + DQ + " + chr(10) + chr(10) + " + DQ + "  regex.test(pattern, text)           # boolean match" + DQ + " + chr(10) + " + DQ + "  regex.search(pattern, text)         # first match {start, end, text}" + DQ + " + chr(10) + " + DQ + "  regex.find_all(pattern, text)       # all matches" + DQ + " + chr(10) + " + DQ + "  regex.replace_all(pattern, text, r) # replace" + DQ + " + chr(10) + " + DQ + "  regex.split_by(pattern, text)       # split" + DQ + " + chr(10) + chr(10) + " + DQ + "Supports: . * + ? [] [^] ^ $ | " + DQ + " + chr(92) + " + DQ + "d " + DQ + " + chr(92) + " + DQ + "w " + DQ + " + chr(92) + " + DQ + "s and negations" + DQ)
emit("    if topic == " + DQ + "graphics" + DQ + ":")
emit("        answer = " + DQ + "Sage GPU engine (lib/graphics/, 24 modules):" + DQ + " + chr(10) + chr(10) + " + DQ + "  import gpu               # Native Vulkan backend (100+ functions)" + DQ + " + chr(10) + " + DQ + "  import graphics.opengl   # OpenGL 4.5 drop-in replacement" + DQ + " + chr(10) + " + DQ + "  import graphics.vulkan   # Builder API helpers" + DQ + " + chr(10) + " + DQ + "  import graphics.math3d   # vec2/3/4, mat4, camera, projection" + DQ + " + chr(10) + " + DQ + "  import graphics.mesh     # Procedural geometry, OBJ loading" + DQ + " + chr(10) + " + DQ + "  import graphics.renderer # Frame loop, depth buffer, sync" + DQ + " + chr(10) + " + DQ + "  import graphics.pbr      # Cook-Torrance PBR materials" + DQ + " + chr(10) + " + DQ + "  + shadows, deferred, taa, scene, material, ui, etc." + DQ)
emit("    # --- General language answers ---")
...
emit("        push(chain["steps"], {"type": "thought", "content": " + DQ + "User wants a plan. Let me break this down." + DQ + ")")
...
emit("    push(chain["steps"], {"type": "thought", "content": " + DQ + "Answering about " + DQ + " + topic)")
emit("    chain["conclusion"] = answer)")
