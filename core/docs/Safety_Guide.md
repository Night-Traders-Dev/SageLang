...
- **Recursion/Loop Limits**: Hard limits on recursion depth and loop iterations protect against infinite execution. Since v4.1.16 a stack-proximity guard additionally tracks real C-stack consumption per thread (budget = 75% of `RLIMIT_STACK`), so deep recursion fails with a catchable exception before the OS stack overflows — regardless of per-frame size. Since v4.2.9 the budget is measured from the *calling thread's* actual stack bounds rather than from `RLIMIT_STACK`: the main stack is raised to 512 MB so it can grow on demand, but worker threads are created with a fixed stack that a later `setrlimit` cannot change, so using the rlimit for a worker overstated its budget and let runaway recursion segfault. Each thread now gets a budget for the stack it really has.
