...
2. **Test Suite Failures**
   - `make test-all` currently fails.
   - The AST interpreter and VM have failing unit tests in `sys_info` (which could be unrelated environment drift) and `arrays_basic`.
...
2. **Fix Failing Tests**
   - Resolve `arrays_basic` and `sys_info` test failures.
