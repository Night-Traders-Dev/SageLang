...
- **v3.9.4 updates**: OIS integration, $O(1)$ dictionary size, $O(N)$ unique checks (simple types), native array reversal, and binary exponentiation for repeating (linear output work).
...
Note that `super` auto-injects `self` as the first argument, but as of v3.9.4, passing it explicitly is also supported for compatibility:
...
Sage v3.9.4 includes a compile-time safety system inspired by Rust. It provides
...
SageLang v3.9.4 expands the `os.boot` library with 20+ new modules for building sophisticated multi-stage bootloaders.
...
## Algorithmic Optimizations (v3.9.4)
...
As of v3.9.4, Sage's default runtime is `auto` — JIT profiling mode on hosted platforms,
...
5. **`match`, `init`, and `end` are reserved keywords** -- **FIXED.** As of v3.9.4, `match`, `init`, `end`, `enum`, `struct`, and `trait` can be used as variable names in expression and assignment contexts, as long as they are not immediately followed by a block-starting colon or used in a declaration context.

6. **`super` auto-injects `self`** -- **FIXED.** As of v3.9.4, you can write either `super.init(args)` or `super.init(self, args)`. The interpreter and backends now handle both forms correctly.
