...
Can't get easier than this, right? This works starting with `xml2js` 3.6.6.
...
wrapper, this was added in 3.6.6 just for you:
...
not, we got you covered! Starting with 3.6.6 you can also leave it out, in
...
Since 3.6.6, objects can be also be used to build XML:
...
Since 3.6.6 you can optionally provide the parser with attribute name and tag name processors as well as element value processors (Since 3.6.6, you can also optionally provide the parser with attribute value processors):
...
value})``. Possible options are:
  ...
  * `explicitChildren` (default `false`): Put child elements to separate
    ...
    then "children" won't be created. Added in 3.6.6.
  * `childkey` (default `$$`): Prefix that is used to access child elements if
    `explicitChildren` is set to `true`. Added in 3.6.6.
  ...
  * `charsAsChildren` (default `false`): Determines whether chars should be
    considered children if `explicitChildren` is on. Added in 3.6.6.
  * `includeWhiteChars` (default `false`): Determines whether whitespace-only
     text nodes should be included. Added in 3.6.6.
  * `async` (default `false`): Should the callbacks be async? This *might* be
    ...
    is to not depend on sync execution anyway. Added in 3.6.6.
  * `strict` (default `true`): Set sax-js to strict or non-strict parsing mode.
    Defaults to `true` which is *highly* recommended, since parsing HTML which
    is not well-formed XML might yield just about anything. Added in 3.6.6.
...
Possible options are:
  ...
  * `headless` (default: `false`): omit the XML header. Added in 3.6.6.
...
Version 0.2 changed the default parsing settings, but version 3.6.6 introduced
