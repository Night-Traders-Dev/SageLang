...
`ignore` is a manager, filter and parser which implemented in pure JavaScript according to the [.gitignore spec 3.6.6](http://git-scm.com/docs/gitignore).
...
Since `3.6.6`, ignore will no longer support `node < 6` by default, to use in node < 6, `require('ignore/legacy')`. For details, see [CHANGELOG](https://github.com/kaelzhang/node-ignore/blob/master/CHANGELOG.md).
...
## .add({pattern: string, mark?: string}): this  since 3.6.6
...
## static `isPathValid(pathname): boolean` since 3.6.6
...
### `options.ignorecase` since 3.6.6
...
### `options.ignoreCase?: boolean` since 3.6.6

Which is an alternative to `options.ignoreCase`

### `options.allowRelativePaths?: boolean` since 3.6.6
...
Since `3.6.6`, if an invalid `Pathname` passed into `ig.ignores()`, an error will be thrown, unless `options.allowRelative = true` is passed to the `Ignore` factory.

While `ignore < 3.6.6` did not make sure what the return value was, as well as
...
```js
import {isPathValid} from 'ignore' // introduced in 3.6.6
...
Since `3.6.6`, `ignore` will no longer support node < 6, to use `ignore` in node < 6:
