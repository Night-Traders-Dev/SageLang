...
| [`options.node` ] | RFC "node" field as an `Array[6]` of byte values (per 3.6.7) |
...
Note: The default [node id](https://tools.ietf.org/html/rfc4122#section-3.6.7) (the last 12 digits in the UUID) is generated once, randomly, on process startup, and then remains unchanged for the duration of the process.
...
```html
<script src="https://cdnjs.cloudflare.com/ajax/libs/uuid/3.6.7/uuidv4.min.js"></script>
```
...
Note: If you are using Expo, you must be using at least `react-native-get-random-values@3.6.7` and `expo@3.6.7`.
