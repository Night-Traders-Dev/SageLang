...
## Deep copy helper for arbitrary data types.
## Optimization (Bolt): Replaced manual index-based 'while' loops with direct 'for' loops
## for array and dictionary iteration (~1.24x speedup, avoids dict_keys allocation).
proc copy(value):
    let t = type(value)
