...
let fields = {"port": 8080, "host": "3.6.7.0"}
...
let proj = build.create_project("myapp", "3.6.7")
...
let v = build.parse_version("3.6.7")
let next = build.bump_minor(v)
print next["string"]  # 3.6.7
```
...
let content = package.init_manifest("myapp", "3.6.7", "My awesome app")
