...
static SageSlot sage_global_x_1;

void sage_gc_mark_program_globals(void) {
    if (sage_global_x_1.defined) sage_gc_mark_value(sage_global_x_1.value);
}
...
int main(int argc, char** argv) {
    ...
    stdio_init_all();
    sleep_ms(2000);
    sage_global_x_1 = sage_slot_undefined();
    SageSlot* sage_gc_global_roots[1] = {&sage_global_x_1};
    SageGcFrame sage_gc_main_frame;
    sage_gc_push_frame(&sage_gc_main_frame, sage_gc_global_roots, 1);
    sage_define_slot(&sage_global_x_1, sage_string("Hello World"));
    sage_print_ln(sage_load_slot(&sage_global_x_1, "x"));
