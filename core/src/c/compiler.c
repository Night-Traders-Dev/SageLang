...
static char *emit_call_expr(Compiler *compiler, CallExpr *call) {
  ...
  if (strcmp(callee_name, "io_appendbytes") == 0) {
    if (call->arg_count != 2)
      return str_dup("sage_nil()");
    char *a1 = emit_expr(compiler, call->args[0]);
    char *a2 = emit_expr(compiler, call->args[1]);
    sb_appendf(&sb, "sage_io_appendbytes(%s, %s)", a1, a2);
    free(a1);
    free(a2);
    free(callee_name);
    return sb_take(&sb);
  ...
  if (strcmp(callee_name, "range") == 0) {
    if (call->arg_count == 1) {
      ...
      char *start_arg = emit_expr(compiler, call->args[0]);
      char *end_arg = emit_expr(compiler, call->args[1]);
      char *step_arg = emit_expr(compiler, call->args[2]);
      sb_appendf(&sb, "sage_range3(%s, %s, %s)", start_arg, end_arg, step_arg);
      free(start_arg);
      free(end_arg);
      free(step_arg);
    } else {
...
"static void sage_gc_mark_value(SageValue value);\n"
       ...
       "    for (int i = 0; i < SAGE_INTERN_CAPACITY; i++) {\n"
       "        if (sage_intern_table[i].content != NULL) {\n"
  ...
  fputs(
      ...
      "    if (!sage_gc.enabled || atomic_load_explicit(&sage_gc.pin_count, memory_order_acquire) > 0) return 0;\n"
      ...
      "\n"
       ...
       "}\n"
       "\n"
       "static void sage_gc_pin(void) { atomic_fetch_add_explicit(&sage_gc.pin_count, 1, memory_order_acq_rel); }\n"
      "static void sage_gc_unpin(void) { int current = atomic_load_explicit(&sage_gc.pin_count, memory_order_acquire); while (current > 0) { if (atomic_compare_exchange_weak_explicit(&sage_gc.pin_count, &current, current - 1, memory_order_acq_rel, memory_order_acquire)) return; } }\n"
      "\n"
  ...
  fputs("typedef struct { int lock; SageValue value; } SageAtomicBox;\n"
        "static void sage_atomic_box_lock(SageAtomicBox* box) {\n"
        "    while (__atomic_exchange_n(&box->lock, 1, __ATOMIC_ACQUIRE) != 0) { }\n"
        "}\n"
        "static void sage_atomic_box_unlock(SageAtomicBox* box) {\n"
        "    __atomic_store_n(&box->lock, 0, __ATOMIC_RELEASE);\n"
        "}\n"
        "static SageValue sage_atomic_new(SageValue val) {\n"
        "    SageAtomicBox* box = (SageAtomicBox*)malloc(sizeof(SageAtomicBox));\n"
        "    if (box == NULL) return sage_nil();\n"
        "    box->lock = 0; box->value = val;\n"
        "    SageValue v; v.type = SAGE_TAG_POINTER; v.as.pointer = box; return v;\n"
        "}\n"
        "static SageValue sage_atomic_load(SageValue atom) {\n"
        "    if (atom.type != SAGE_TAG_POINTER || atom.as.pointer == NULL) return sage_nil();\n"
        "    SageAtomicBox* box = (SageAtomicBox*)atom.as.pointer;\n"
        "    sage_atomic_box_lock(box);\n"
        "    SageValue result = box->value;\n"
        "    sage_atomic_box_unlock(box);\n"
        "    return result;\n"
        "}\n"
        ...
        "    return val;\n"
        "}\n"
        "static SageValue sage_atomic_add(SageValue atom, SageValue val) {\n"
        "    if (atom.type != SAGE_TAG_POINTER || atom.as.pointer == NULL || val.type != SAGE_TAG_NUMBER) return sage_nil();\n"
        "    SageAtomicBox* box = (SageAtomicBox*)atom.as.pointer;\n"
        "    sage_atomic_box_lock(box);\n"
        "    SageValue old = box->value;\n"
        "    if (old.type == SAGE_TAG_NUMBER) box->value = sage_number(old.as.number + val.as.number);\n"
        "    sage_atomic_box_unlock(box);\n"
        "    return old;\n"
        "}\n"
        "static SageValue sage_atomic_cas(SageValue atom, SageValue old, SageValue new_val) {\n"
        "    if (atom.type != SAGE_TAG_POINTER || atom.as.pointer == NULL) return sage_bool(0);\n"
        "    SageAtomicBox* box = (SageAtomicBox*)atom.as.pointer;\n"
        "    sage_atomic_box_lock(box);\n"
        "    int matches = box->value.type == old.type && box->value.type == SAGE_TAG_NUMBER && old.type == SAGE_TAG_NUMBER && box->value.as.number == old.as.number;\n"
        "    if (matches) box->value = new_val;\n"
        "    sage_atomic_box_unlock(box);\n"
        "    return sage_bool(matches);\n"
        "}\n"
        "static SageValue sage_atomic_exchange(SageValue atom, SageValue val) {\n"
        "    if (atom.type != SAGE_TAG_POINTER || atom.as.pointer == NULL) return sage_nil();\n"
        "    SageAtomicBox* box = (SageAtomicBox*)atom.as.pointer;\n"
        "    sage_atomic_box_lock(box);\n"
        "    SageValue old = box->value;\n"
  ...
  fputs(
      ...
      "    if (needed <= 0 || needed > 1073741823) sage_fail(\"Runtime Error: out of memory\");\n"
      "    if (array->capacity >= needed) return;\n"
      "    int capacity = array->capacity == 0 ? 4 : array->capacity;\n"
      "    while (capacity < needed) {\n"
      "        if (capacity > 536870911) sage_fail(\"Runtime Error: out of memory\");\n"
      "        capacity *= 2;\n"
      "    }\n"
  ...
  fputs("#if !defined(PICO_ON_DEVICE)\n"
      ...
      "        if (!data) { fclose(f); return sage_nil(); }\n"
      "        if (size > 0 && fread(data, 1, (size_t)size, f) != (size_t)size) { free(data); fclose(f); return sage_nil(); }\n"
      "        fclose(f);\n"
      "        SageBytes* bytes = (SageBytes*)malloc(sizeof(SageBytes));\n"
      "        if (!bytes) { free(data); return sage_nil(); }\n"
      ...
      "        if (total_read + nread > SAGE_MAX_READ_SIZE) nread = SAGE_MAX_READ_SIZE - total_read;\n"
...
"    if (!f) return sage_nil();\n"
"    if (v.type == SAGE_TAG_BYTES) {\n"
"        if (v.as.bytes->count > 0) fwrite(v.as.bytes->data, 1, v.as.bytes->count, f);\n"
"        fclose(f);\n"
"        return sage_bool(1);\n"
"    }\n"
"    if (v.type != SAGE_TAG_ARRAY) { fclose(f); return sage_nil(); }\n"
"    SageArray* a = v.as.array;\n"
"    unsigned char chunk[4096];\n"
"    int pos = 0;\n"
"    for (int i = 0; i < a->count; i++) {\n"
"        chunk[pos++] = (unsigned char)a->elements[i].as.number;\n"
"        if (pos == sizeof(chunk)) {\n"
...
"    return sage_bool(1);\n"
"}\n"
"static SageValue sage_native_io_appendbytes(SageValue path, SageValue v) {\n"
"    if (path.type != SAGE_TAG_STRING) return sage_nil();\n"
...
"#endif\n"
  ...
  fputs(
      ...
      "    if (!isfinite(start.as.number) || !isfinite(end.as.number) || floor(start.as.number) != start.as.number || floor(end.as.number) != end.as.number) return sage_nil();\n"
      "    long long first = (long long)start.as.number, last = (long long)end.as.number;\n"
      "    if (last < first || last - first > 10000000) return sage_nil();\n"
      "    sage_gc_pin();\n"
      "    SageValue result = sage_array();\n"
      ...
      "    if (!isfinite(start.as.number) || !isfinite(end.as.number) || !isfinite(step.as.number)) return sage_nil();\n"
      "    if (floor(start.as.number) != start.as.number || floor(end.as.number) != end.as.number || floor(step.as.number) != step.as.number) return sage_nil();\n"
      "    long long s = (long long)start.as.number, e = (long long)end.as.number, st = (long long)step.as.number;\n"
      "    if (st == 0) return sage_nil();\n"
      "    unsigned long long steps = st > 0 ? (e > s ? (unsigned long long)(e - s) : 0) : (s > e ? (unsigned long long)(s - e) : 0);\n"
      "    if (steps > 10000000) return sage_nil();\n"
      "    sage_gc_pin();\n"
      "    SageValue result = sage_array();\n"
      "    if (st > 0) {\n"
  ...
  fputs(
      ...
      "    if (left.type != SAGE_TAG_NUMBER || right.type != SAGE_TAG_NUMBER) "
      "sage_fail(\"Runtime Error: Operands must be numbers.\");\n"
      "    return sage_number((double)(((unsigned long long)left.as.number) << ((long "
      "long)right.as.number)));\n"
      "}\n"
  ...
  fputs(
      ...
      "    if (s.type != SAGE_TAG_STRING || count.type != SAGE_TAG_NUMBER || !s.as.string) return sage_nil();\n"
      "    double requested = count.as.number;\n"
      "    if (!isfinite(requested) || requested < 1.0 || floor(requested) != requested || requested > 104857600.0) return sage_nil();\n"
      "    size_t slen = SAGE_STRING_LEN(s);\n"
      "    if (slen == 0) return sage_string(\"\");\n"
      "    size_t n = (size_t)requested;\n"
      "    if (slen > 104857600 / n || slen * n > 104857599) return sage_nil();\n"
      "    char* buf = malloc(slen * n + 1);\n"
      "    if (!buf) return sage_nil();\n"
      "    for (size_t i = 0; i < n; i++) memcpy(buf + i * slen, s.as.string, slen);\n"
      "    buf[slen * n] = '\\0';\n"
      "    SageValue result = sage_string_take(buf);\n"
      "    return result;\n"
      "}\n"
  ...
  fputs("static void sage_index_set(SageValue c, SageValue k, SageValue v) {\n"
        ...
        "            if (d->capacity > 1073741823) sage_fail(\"Runtime Error: out of memory\");\n"
        "            int nc = d->capacity == 0 ? 4 : d->capacity * 2;\n"
        "            char** new_keys = (char**)malloc(sizeof(char*) * (size_t)nc);\n"
        "            SageValue* new_values = (SageValue*)malloc(sizeof(SageValue) * (size_t)nc);\n"
        "            if (new_keys == NULL || new_values == NULL) { free(new_keys); free(new_values); sage_fail(\"Runtime Error: out of memory\"); }\n"
        "            if (d->count > 0) {\n"
        "                memcpy(new_keys, d->keys, sizeof(char*) * (size_t)d->count);\n"
        "                memcpy(new_values, d->values, sizeof(SageValue) * (size_t)d->count);\n"
        "            }\n"
        "            free(d->keys); free(d->values);\n"
        "            d->keys = new_keys; d->values = new_values; d->capacity = nc;\n"
        "        }\n"
        "        { size_t l = SAGE_STRING_LEN(k); d->keys[d->count] = (char*)malloc(l+1); if (d->keys[d->count] == NULL) sage_fail(\"Runtime Error: out of memory\"); memcpy(d->keys[d->count], k.as.string, l+1); }\n"
        "        d->values[d->count] = v;\n"
        "        d->count++;\n"
        "    }\n"
        "}\n"
  ...
  fputs(
      ...
      "        size_t part_len = arr->elements[i].type == SAGE_TAG_STRING ? strlen(arr->elements[i].as.string) : 0;\n"
      "        size_t extra = i > 0 ? dlen : 0;\n"
      "        if (part_len > SAGE_MAX_READ_SIZE - total || extra > SAGE_MAX_READ_SIZE - total - part_len) return sage_nil();\n"
      "        total += part_len + extra;\n"
      "    }\n"
  ...
  fputs("static SageValue sage_replace_fn(SageValue str_val, SageValue "
        ...
        "        if (count > SAGE_MAX_READ_SIZE / delta) return sage_nil();\n"
        "        size_t extra = count * delta;\n"
        "        if (source_len > SAGE_MAX_READ_SIZE - extra) return sage_nil();\n"
        "        result_len = source_len + extra;\n"
        "    } else if (new_len < old_len) {\n"
        "        size_t delta = old_len - new_len;\n"
        "        if (count > source_len / delta) return sage_nil();\n"
  ...
  if (target != COMPILER_TARGET_RP2040 && target != COMPILER_TARGET_RP2350_ARM && target != COMPILER_TARGET_RP2350_RISCV) {
    fputs(
        ...
        "sage_bool(1);\n"
        "}\n"
        "static SageValue sage_io_writebytes(SageValue p, SageValue v) {\n"
        "    if(p.type != SAGE_TAG_STRING || (v.type != SAGE_TAG_BYTES && v.type != SAGE_TAG_ARRAY)) return sage_bool(0);\n"
        "    FILE* f = fopen(p.as.string, \"wb\"); if(!f) return "
        "sage_bool(0);\n"
        "    if(v.type == SAGE_TAG_BYTES) {\n"
        "        if(v.as.bytes && v.as.bytes->data && v.as.bytes->count > 0) {\n"
        "            fwrite(v.as.bytes->data, 1, v.as.bytes->count, f);\n"
        "        }\n"
        "        fclose(f); return sage_bool(1);\n"
        "    }\n"
        "    if(!v.as.array) { fclose(f); return sage_bool(0); }\n"
        "    SageArray* a = v.as.array;\n"
        "    unsigned char chunk[4096]; int pos = 0;\n"
        "    for(int i=0; i<a->count; i++) {\n"
        "        chunk[pos++] = (unsigned char)a->elements[i].as.number;\n"
        "        if(pos == (int)sizeof(chunk)) {\n"
        "            fwrite(chunk, 1, sizeof(chunk), f); pos = 0;\n"
        "        }\n"
        "    }\n"
        "    if(pos > 0) fwrite(chunk, 1, pos, f);\n"
        "    fclose(f); return sage_bool(1);\n"
        "}\n"
        "static SageValue sage_io_appendbytes(SageValue p, SageValue v) {\n"
        "    if(p.type != SAGE_TAG_STRING || (v.type != SAGE_TAG_BYTES && v.type != SAGE_TAG_ARRAY)) return sage_bool(0);\n"
        "    FILE* f = fopen(p.as.string, \"ab\"); if(!f) return "
        "sage_bool(0);\n"
        "    if(v.type == SAGE_TAG_BYTES) {\n"
        "        if(v.as.bytes && v.as.bytes->data && v.as.bytes->count > 0) {\n"
        "            fwrite(v.as.bytes->data, 1, v.as.bytes->count, f);\n"
