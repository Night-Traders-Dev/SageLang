...
char* aot_compile_program(AotCompiler* aot, Stmt* program) {
    ...
    aot_emit(aot, "static SageValue s_lower(int c, SageValue* a) { if(c<1||a[0].type!=SAGE_STR||!a[0].as.string)return sage_string(\"\"); char*s=strdup(a[0].as.string); if(!s)return sage_string(\"\"); for(int i=0;s[i];i++)if(s[i]>='A'&&s[i]<='Z')s[i]+=32; return sage_string(s); }");
