...
char* aot_compile_program(AotCompiler* aot, Stmt* program) {
    ...
    aot_emit(aot, "static SageValue s_join(int c, SageValue* a) { if(c>0&&a[0].type==6){pthread_join(*(pthread_t*)a[0].as.ptr, NULL); free(a[0].as.ptr); return sage_nil();} if(c>1&&a[0].type==SAGE_ARR&&a[1].type==SAGE_STR){SageArr*arr=(SageArr*)a[0].as.ptr; if(!arr||arr->count<=0)return sage_string(\"\"); size_t sep_len=strlen(a[1].as.string); size_t total_len=0; for(int i=0;i<arr->count;i++){SageValue sa=sage_str(arr->elems[i]); size_t sl=strlen(sa.as.string); if(total_len+sl<total_len||total_len+sl>104857600)return sage_string(\"\"); total_len+=sl;} if(arr->count>1){if(sep_len>0&&(size_t)(arr->count-1)>(104857600-total_len)/sep_len)return sage_string(\"\"); total_len+=sep_len*(size_t)(arr->count-1);} char*b=malloc(total_len+1); if(!b)return sage_string(\"\"); char*wp=b; for(int i=0;i<arr->count;i++){SageValue sa=sage_str(arr->elems[i]); size_t sl=strlen(sa.as.string); if(sl>0){memcpy(wp,sa.as.string,sl);wp+=sl;} if(i<arr->count-1&&sep_len>0){memcpy(wp,a[1].as.string,sep_len);wp+=sep_len;}} *wp=0; SageValue v; v.type=SAGE_STR; v.as.string=b; return v;} return sage_nil(); }");
