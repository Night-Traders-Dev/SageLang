...
char* aot_compile_program(AotCompiler* aot, Stmt* program) {
    ...
    // Hardened against CWE-400 (resource exhaustion) and CWE-476 (NULL pointer dereference)
    aot_emit(aot, "static SageValue s_recv(int c, SageValue* a) { if(c<2||a[0].type!=SAGE_NUM||a[1].type!=SAGE_NUM)return sage_string(\"\"); int fd=(int)a[0].as.number; int size=(int)a[1].as.number; if(size<=0||size>104857600)return sage_string(\"\"); char*buf=malloc(size+1); if(!buf)return sage_string(\"\"); int r=recv(fd,buf,size,0); if(r<=0){free(buf);return sage_string(\"\");} buf[r]=0; SageValue v=sage_string(buf); free(buf); return v; }");
