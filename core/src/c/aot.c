...
char* aot_compile_program(AotCompiler* aot, Stmt* program) {
    ...
    aot_emit(aot, "static SageValue s_replace(int c, SageValue* a) { if(c<3||a[0].type!=SAGE_STR||a[1].type!=SAGE_STR||a[2].type!=SAGE_STR)return a[0]; const char*str=a[0].as.string; const char*f=a[1].as.string; const char*r=a[2].as.string; size_t flen=strlen(f); if(flen==0)return a[0]; size_t rlen=strlen(r); size_t count=0; const char*p=str; while((p=strstr(p,f))!=NULL){count++; p+=flen;} size_t slen=strlen(str); size_t newlen=slen+count*(rlen-flen); if(newlen>104857600)return a[0]; char*b=malloc(newlen+1); if(!b)return a[0]; char*out=b; p=str; const char*m; while((m=strstr(p,f))!=NULL){size_t prefix=m-p; memcpy(out,p,prefix); out+=prefix; memcpy(out,r,rlen); out+=rlen; p=m+flen;} strcpy(out,p); SageValue v; v.type=SAGE_STR; v.as.string=b; return v; }");
