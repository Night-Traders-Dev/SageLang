    ...
    switch (expr->type) {
        ...
        case EXPR_CALL: {
            ...
            if (raw) {
                ...
                if (BUILTIN_MATCH("repeat") && argc==2) EMIT_2("sage_repeat_value");
                ...
                if (is_proc && (!expr->as.call.callee || expr->as.call.callee->type != EXPR_GET)) {
                    pos += sprintf(buf + pos, "%s(%d, _args); })", fname, argc);
                } else if (expr->as.call.callee && expr->as.call.callee->type == EXPR_GET) {
                    char* receiver = aot_compile_expr(aot, expr->as.call.callee->as.get.object);
                    char* callee_expr = aot_compile_expr(aot, expr->as.call.callee);
                    size_t required = strlen(receiver) + strlen(callee_expr) + 64;
                    if ((size_t)pos + required + 1 > total_len) {
                        char* grown = realloc(buf, pos + required + 1);
                        if (!grown) {
                            free(receiver);
                            free(callee_expr);
                            free(fname);
                    ...
                    pos += sprintf(buf + pos, "sage_call_with_receiver(%s, %s, %d, _args); })", receiver, callee_expr, argc);
                    free(receiver);
                    free(callee_expr);
                } else {
        ...
        case EXPR_SET: {
            if (expr->as.set.object && expr->as.set.property.start) {
                char* obj = aot_compile_expr(aot, expr->as.set.object);
                char* prop = sanitize_name(expr->as.set.property.start, expr->as.set.property.length);
                char* val = aot_compile_expr(aot, expr->as.set.value);
                char* buf = malloc(strlen(obj) + strlen(prop) + strlen(val) + 64);
                if (buf) {
                    sprintf(buf, "sage_index_set(%s, sage_string(\"%s\"), %s)", obj, prop + 2, val);
                } else {
                ...
                if (m->type == STMT_PROC) {
                    ...
                    for (int i = 0; i < m->as.proc.param_count; i++) {
                        ...
                        if (strcmp(pn, "s_self") != 0 && strcmp(pn, "v_s_self") != 0) {
                            aot_emit(aot, "SageValue %s = (argc > %d) ? argv[%d] : sage_nil();", pn, arg_index, arg_index);
                            arg_index++;
                        }
                        free(pn);
                    }
...
char* aot_compile_program(AotCompiler* aot, Stmt* program) {
    ...
    aot_emit(aot, "static SageValue sage_strcat(SageValue a, SageValue b) { if(a.type!=SAGE_STR||b.type!=SAGE_STR) return sage_nil(); if(!a.as.string||!b.as.string) return sage_nil(); size_t la=strlen(a.as.string),lb=strlen(b.as.string); if(la>104857599||lb>104857599||la>104857599-lb) return sage_nil(); char* r=malloc(la+lb+1); if(!r) return sage_nil(); memcpy(r,a.as.string,la); memcpy(r+la,b.as.string,lb); r[la+lb]=0; SageValue v; v.type=SAGE_STR; v.as.string=r; return v; }");
    aot_emit(aot, "");
    ...
    aot_emit(aot, "static SageValue sage_call_with_receiver(SageValue receiver, SageValue method, int argc, SageValue* args) { SageValue previous = s_current_self; s_current_self = receiver; SageValue result = sage_call(method, argc, args); s_current_self = previous; return result; }");
    ...
    aot_emit(aot, "static SageValue s_lower(int c, SageValue* a) { if(c<1||a[0].type!=SAGE_STR||!a[0].as.string)return sage_string(\"\"); char*s=strdup(a[0].as.string); if(!s)return sage_string(\"\"); for(int i=0;s[i];i++)if(s[i]>='A'&&s[i]<='Z')s[i]+=32; SageValue v=sage_string(s); free(s); return v; }");
    aot_emit(aot, "#include <ctype.h>");
    aot_emit(aot, "static int s_is_safe_command(const char* cmd) { if(!cmd) return 0; while(*cmd&&isspace((unsigned char)*cmd))cmd++; if(cmd[0]=='-') return 0; for(const char* p=cmd;*p;p++) { if(!isalnum((unsigned char)*p)&&*p!='/'&&*p!='.'&&*p!='-'&&*p!='_'&&*p!='~'&&*p!=' '&&*p!='\\'') return 0; } return 1; }");
    aot_emit(aot, "static SageValue s_shell_exec(int c, SageValue* a) { if(c<1||a[0].type!=SAGE_STR)return sage_string(\"\"); if(!s_is_safe_command(a[0].as.string)){fprintf(stderr, \"Security Error: Unsafe characters in command\\n\"); return sage_string(\"\");} FILE*f=popen(a[0].as.string,\"r\"); if(!f)return sage_string(\"\"); char buf[4096]; char*out=NULL; size_t slen=0; while(fgets(buf,sizeof(buf),f)){size_t l=strlen(buf); if(slen+l>104857600){l=104857600-slen;if(l==0)break;} char* nout=realloc(out,slen+l+1); if(!nout)break; out=nout; memcpy(out+slen,buf,l); slen+=l; out[slen]=0;} pclose(f); SageValue v=sage_string(out?out:\"\"); free(out); return v; }");
    ...
    aot_emit(aot, "static SageValue s_recv(int c, SageValue* a) { if(c<2||a[0].type!=SAGE_NUM||a[1].type!=SAGE_NUM)return sage_string(\"\"); double fd_value=a[0].as.number; double size_value=a[1].as.number; if(!isfinite(fd_value)||floor(fd_value)!=fd_value||fd_value<0||fd_value>INT_MAX)return sage_string(\"\"); if(!isfinite(size_value)||floor(size_value)!=size_value||size_value<=0||size_value>104857600)return sage_string(\"\"); int fd=(int)fd_value; size_t size=(size_t)size_value; char*buf=malloc(size+1); if(!buf)return sage_string(\"\"); ssize_t r=recv(fd,buf,size,0); if(r<=0){free(buf);return sage_string(\"\");} buf[r]=0; SageValue v=sage_string(buf); free(buf); return v; }");
    ...
    aot_emit(aot, "static SageValue s_join(int c, SageValue* a) { if(c>0&&a[0].type==6){if(a[0].as.ptr){pthread_join(*(pthread_t*)a[0].as.ptr, NULL); free(a[0].as.ptr); a[0].as.ptr=NULL;} return sage_nil();} if(c>1&&a[0].type==SAGE_ARR&&a[1].type==SAGE_STR){SageArr*arr=(SageArr*)a[0].as.ptr; if(!arr||arr->count==0)return sage_string(\"\"); size_t dlen=strlen(a[1].as.string); size_t len=0; for(int i=0;i<arr->count;i++){SageValue sa=sage_str(arr->elems[i]); size_t elen=strlen(sa.as.string); if(len+elen<len||len+elen>104857600)return sage_string(\"\"); len+=elen; if(i<arr->count-1){if(len+dlen<len||len+dlen>104857600)return sage_string(\"\"); len+=dlen;}} char*b=malloc(len+1); if(!b)return sage_string(\"\"); char*wp=b; for(int i=0;i<arr->count;i++){SageValue sa=sage_str(arr->elems[i]); size_t elen=strlen(sa.as.string); memcpy(wp,sa.as.string,elen); wp+=elen; if(i<arr->count-1){memcpy(wp,a[1].as.string,dlen); wp+=dlen;}} *wp=0; SageValue v; v.type=SAGE_STR; v.as.string=b; return v;} return sage_nil(); }");
    ...
    aot_emit(aot, "static SageValue s_string_repeat(int c, SageValue* a) { if(c<2||a[0].type!=SAGE_STR||a[1].type!=SAGE_NUM||!a[0].as.string)return sage_string(\"\"); double requested=a[1].as.number; if(!isfinite(requested)||requested<1.0||floor(requested)!=requested||requested>104857600.0)return sage_string(\"\"); size_t l=strlen(a[0].as.string); if(l==0)return sage_string(\"\"); size_t t=(size_t)requested; if(l>104857599/t||l*t>104857599)return sage_string(\"\"); char*b=malloc(l*t+1); if(!b)return sage_string(\"\"); for(size_t i=0;i<t;i++)memcpy(b+i*l,a[0].as.string,l); b[l*t]=0; return sage_string(b); }");
    aot_emit(aot, "static SageValue sage_repeat_value(SageValue s, SageValue n) { SageValue args[2]; args[0]=s; args[1]=n; return s_string_repeat(2,args); }");
    ...
    aot_emit(aot, "static SageValue s_readfile(int c, SageValue* a) { if(c<1||a[0].type!=SAGE_STR)return sage_string(\"\"); FILE* f=fopen(a[0].as.string,\"rb\"); if(!f)return sage_string(\"\"); fseek(f,0,SEEK_END); long s=ftell(f); fseek(f,0,SEEK_SET); if(s<0||s>104857600){fclose(f);return sage_string(\"\");} char* b=malloc(s+1); if(!b){fclose(f);return sage_string(\"\");} size_t r=fread(b,1,s,f); b[r]=0; fclose(f); return sage_string(b); }");
    ...
    aot_emit(aot, "static SageValue s_split(int c, SageValue* a) { if(c<2||a[0].type!=SAGE_STR||a[1].type!=SAGE_STR||!a[0].as.string||!a[1].as.string)return sage_array(0); SageArr*r=malloc(sizeof(SageArr)); if(!r)return sage_array(0); r->cap=16; r->count=0; r->elems=malloc(sizeof(SageValue)*r->cap); if(!r->elems){free(r);return sage_array(0);} char*str=strdup(a[0].as.string); if(!str){free(r->elems);free(r);return sage_array(0);} char*tok=strtok(str,a[1].as.string); while(tok){if(r->count>=r->cap){int nc=r->cap*2; if(nc<r->cap||nc>1000000)break; SageValue*ne=realloc(r->elems,sizeof(SageValue)*nc); if(!ne)break; r->elems=ne; r->cap=nc;} char*st=strdup(tok); if(!st)break; r->elems[r->count++]=sage_string(st); tok=strtok(NULL,a[1].as.string);} free(str); SageValue v;v.type=SAGE_ARR;v.as.ptr=r;return v; }");
    ...
    aot_emit(aot, "static SageValue s_replace(int c, SageValue* a) { if(c<3||a[0].type!=SAGE_STR||a[1].type!=SAGE_STR||a[2].type!=SAGE_STR)return a[0]; const char*str=a[0].as.string; const char*f=a[1].as.string; const char*r=a[2].as.string; size_t flen=strlen(f); if(flen==0)return a[0]; size_t slen=strlen(str); size_t rlen=strlen(r); size_t count=0; const char*p=str; while((p=strstr(p,f))!=NULL){count++; p+=flen;} size_t newlen=slen; if(count>0){if(rlen>=flen){size_t diff=rlen-flen; if(diff>0&&(slen>104857600||count>(104857600-slen)/diff))return a[0]; newlen=slen+count*diff;}else{size_t diff=flen-rlen; newlen=slen-count*diff;}} if(newlen>104857600)return a[0]; char*b=malloc(newlen+1); if(!b)return a[0]; char*wp=b; p=str; while(1){const char*m=strstr(p,f); if(!m){size_t rem=strlen(p); memcpy(wp,p,rem); wp+=rem; break;} size_t seg=m-p; memcpy(wp,p,seg); wp+=seg; memcpy(wp,r,rlen); wp+=rlen; p=m+flen;} *wp=0; SageValue v; v.type=SAGE_STR; v.as.string=b; return v; }");
