...
char* aot_compile_program(AotCompiler* aot, Stmt* program) {
    ...
    aot_emit(aot, "static int s_is_safe_command(const char* cmd) { if(!cmd) return 1; while(*cmd&&isspace((unsigned char)*cmd))cmd++; if(cmd[0]=='-') return 0; for(const char* p=cmd;*p;p++) { if(!isalnum((unsigned char)*p)&&*p!='/'&&*p!='.'&&*p!='-'&&*p!='_'&&*p!='~'&&*p!=' '&&*p!='\\'') return 0; } return 1; }");
    ...
    aot_emit(aot, "static SageValue s_string_repeat(int c, SageValue* a) { if(c<2||a[0].type!=SAGE_STR||a[1].type!=SAGE_NUM)return sage_nil(); int t=(int)a[1].as.number; if(t<=0)return sage_string(\"\"); size_t l=strlen(a[0].as.string); if(l>0&&(size_t)t>104857600/l)return sage_nil(); char*b=malloc(l*t+1); if(!b)return sage_nil(); for(size_t i=0;i<(size_t)t;i++)memcpy(b+i*l,a[0].as.string,l); b[l*t]=0; SageValue v=sage_string(b); return v; }");
