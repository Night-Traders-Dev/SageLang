...
proc emit_runtime_prelude(cc):
    ...
    push(o, "    if (value.type == SAGE_TAG_NUMBER) return value.as.number != 0.0;" + NL)
    push(o, "    if (value.type == SAGE_TAG_STRING) return value.as.string != NULL && " +
        "value.as.string[0] != '" + bs0 + "';" + NL)
    push(o, "    return 1;" + NL)
    push(o, "}" + NL)
    push(o, NL)
    ...
    push(o, "}" + NL)
    push(o, NL)
    push(o, "static void sage_format_number(double n, char* buf, size_t bufsize) {" + NL)
    push(o, "    if (isfinite(n) && n >= -9007199254740992.0 && " +
        "n <= 9007199254740992.0 && n == (double)(long long)n) {" + NL)
    push(o, "        snprintf(buf, bufsize, " + DQ + "%lld" + DQ + ", (long long)n);" + NL)
    push(o, "        return;" + NL)
    push(o, "    }" + NL)
    push(o, "    for (int prec = 15; prec <= 17; prec++) {" + NL)
    push(o, "        snprintf(buf, bufsize, " + DQ + "%.*g" + DQ + ", prec, n);" + NL)
    push(o, "        if (strtod(buf, NULL) == n) return;" + NL)
    push(o, "    }" + NL)
    push(o, "}" + NL)
    push(o, NL)
    # Print
    ...
    push(o, "            sage_format_number(value.as.number, buffer, sizeof(buffer));" + NL)
    push(o, "            printf(" + DQ + "%s" + DQ + "," +
        " buffer);" + NL)
    push(o, "            break;" + NL)
    push(o, "        }" + NL)
